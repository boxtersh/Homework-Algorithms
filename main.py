...
def max_in_range(arr: list[int | float], start: int, end: int) -> (float | int, int, int):
    ...
    vld.__check_list(arr), vld.__check_list_two_or_more_elements(arr), vld.__check_int(start)
    vld.__check_positive_int(start), vld.__check_int(end), vld.__check_positive_int(end)
    vld.__check_index_belongs_list(arr, start, end), vld.__check_start_less_end(start, end)
...
def rotate_and_reverse(arr: list[int | float], k: int) -> list:
    ...
    vld.__check_list(arr), vld.__check_list_two_or_more_elements(arr)
    vld.__check_int(k), vld.__check_positive_int(k)
...
def reverse_even_elements(arr: list[int | float]) -> list:
    ...
    for i in range(len_even // 2):
        new_arr[list_i_even[i]], new_arr[list_i_even[len_even - 1 - i]] = new_arr[list_i_even[len_even - 1 - i]], \
        new_arr[list_i_even[i]]
